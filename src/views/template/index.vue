    ...
    <div class="table" style="padding:10px;">
      <el-table
        ...
        <el-table-column label="打印" >
          <template slot-scope="{row}">
            <el-button @click="printGood(row)" type="success" size="mini" style="width:80px;">打印</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  methods: {
    ...
    printGood(row){
      this.goodinfo=row;
      var url='http://stbk.jesport.com/index.html#/template?id='+row.id;
      this.link=url;
    ...
    getList(){
      this.postAxios('template/list').then(res=>{
